        ...
        (&Method::POST, "/compute") => {
            ...
            if rate_response.status().is_success() {
                ...
                if rate_response.status() == StatusCode::NOT_FOUND {
                    ...
                    Ok(response_build(&String::from("{\"status\":\"error\",\"message\":\"Unknown error from 'sales_tax_rate' service.\"}")))
                }
            }
        }
